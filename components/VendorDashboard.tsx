...
const VendorDashboard: React.FC<{ orders: Order[], onUpdate: (id: string, s: OrderStatus) => void }> = ({ orders, onUpdate }) => {
  return (
    <div className="animate-in fade-in duration-500">
      <header className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-3xl font-bold">Incoming Orders</h2>
          <p className="text-slate-400">Live queue management for Central Canteen</p>
        </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                  order.type === 'canteen' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'
                }`}>
                  {order.type}
                </span>
